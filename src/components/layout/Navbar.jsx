      ...
      <div className="flex gap-6 items-center">
        ...
        {loading ? (
          ...
          <div className="relative">
            <button onClick={() => setShowDropdown(!showDropdown)} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-dark-900/50 transition-colors">
            ...
            {showDropdown && (
              <div className="absolute right-0 top-full mt-2 w-48 bg-dark-900 border border-brand/20 rounded-lg shadow-lg overflow-hidden">
                <Link to="/profile" className="flex items-center gap-3 px-4 py-3 text-gray-100 hover:bg-dark-800 transition-colors border-b border-brand/10" onClick={() => setShowDropdown(false)}>
                  <User size={16} /><span className="text-sm">Profile</span>
                </Link>
                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-slate-400 hover:text-red-400 hover:bg-dark-800 transition-colors text-left">
          ...
          <Link to="/auth" className="relative overflow-hidden px-5 py-2 rounded-lg bg-gradient-to-r from-brand to-violet-600 text-white text-sm font-semibold hover:-translate-y-0.5 hover:shadow-lg hover:shadow-brand/30 transition-all">
            Sign Up Free
          </Link>
        )}
      </div>
